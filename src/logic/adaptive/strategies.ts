...
export const TEST_STRATEGIES: Record<StrategyName, AdaptiveStrategy> = {
  ...
  HYBRID_STANDARD: {
    ...
    shouldMoveUp: (history) => {
      ...
      return recent.filter((v) => v === true).length >= 2
    },
    shouldMoveDown: (history) => {
      if (history.length < 3) return false
      return history.slice(-3).every((v) => v === false)
    },
  },
  ...
  FAST_TRACK: {
    ...
    shouldMoveUp: (history) => {
      if (history.length < 2) return false
      return history.slice(-2).every((v) => v === true)
    },
    shouldMoveDown: (history) => {
      if (history.length < 2) return false
      return history.slice(-2).every((v) => v === false)
    },
  },
