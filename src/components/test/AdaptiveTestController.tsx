      ...
      <div className='min-h-[500px] flex flex-col'>
        {loading || isSaving || !currentQuestion ? (
          ...
          <div className='animate-in fade-in duration-500 space-y-6'>
            ...
            {(currentQuestion.q_type === 'audio' ||
              ...
              currentQuestion.audio_url && (
                <div className='bg-zinc-50 p-8 rounded-2xl border mb-6 max-w-xl mx-auto w-full text-center space-y-4 shadow-sm'>
                  <audio
                    id={`audio-${currentQuestion.id}`}
                    key={currentQuestion.audio_url}
                    onEnded={() => {
                      setMediaPlaying(false)
                      setPlayedMediaIds((prev) => [...prev, currentQuestion.id])
                    }}
                  >
                  ...
                  <div className='flex justify-center items-center py-2'>
                    <button
                      disabled={
                        mediaPlaying ||
                        playedMediaIds.includes(currentQuestion.id)
                      }
                      className={`relative flex items-center justify-center w-20 h-20 rounded-full transition-all duration-300 shadow-md ${playedMediaIds.includes(currentQuestion.id)
                    ...
                    >
                      {/* Animated Ripple Ring for Active Playback */}
                      {mediaPlaying && (
            ...
            {/* DEDICATED VIDEO COMPONENT VIEW (PLAY ONCE) */}
            {currentQuestion.q_type === 'video' &&
              currentQuestion.video_url && (
                <div className='bg-zinc-950 rounded-2xl border shadow-md max-w-2xl mx-auto w-full overflow-hidden mb-6 relative group aspect-video'>
                  ...
                  {/* Absolute overlay when not playing or completed */}
                  {(!mediaPlaying ||
                    playedMediaIds.includes(currentQuestion.id)) && (
                      <div className='absolute inset-0 bg-black/50 backdrop-blur-[1px] flex flex-col items-center justify-center p-4 transition-all'>
                        <button
                          disabled={playedMediaIds.includes(currentQuestion.id)}
                          className={`flex items-center justify-center w-20 h-20 rounded-full transition-all duration-300 shadow-xl ${playedMediaIds.includes(currentQuestion.id)
                            ? 'bg-zinc-800 text-zinc-500 cursor-not-allowed shadow-none'
                            : 'bg-amber-500 hover:bg-amber-600 text-white hover:scale-105 active:scale-95'
                            }`}
                          onClick={() => {
                            const videoEl = document.getElementById(
                              `video-${currentQuestion.id}`,
                            ) as HTMLVideoElement
                            if (videoEl) {
                              videoEl.play()
                              setMediaPlaying(true)
                  ...
                  {/* Small active indicator while playing */}
                  {mediaPlaying &&
                    !playedMediaIds.includes(currentQuestion.id) && (
